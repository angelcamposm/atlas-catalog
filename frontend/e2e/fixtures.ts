 ...
 * Generated by the global setup (`e2e/global-setup.ts`) when a real
 * backend is available.  Falls back gracefully when the file doesn't
 * exist (e.g., CI without a running backend).
 */
...
export const test = base.extend<{
    ...
    authenticatedPage: async ({ page }, use) => {
        // TODO: Implement actual login flow in global-setup.ts so that
        //       storageState is pre-populated before the test suite runs.
        // For now just navigate to the app root.
        await page.goto("/es");
        await use(page);
    },
});
