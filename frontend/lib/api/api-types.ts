...
export const apiTypesApi = {
    ...
    getAll: async (page = 1): Promise<PaginatedApiTypeResponse> => {
        const response = await apiClient.get<unknown>(
            `/v1/catalog/apis/types${apiClient.buildQuery({ page })}`
        );
     ...
     * Delete an API Type
     */
    delete: (id: number) => apiClient.delete(`/v1/catalog/apis/types/${id}`),
};
